class c43br3c5 {
    constructor(config = {}) {
        ...
        this.wtEndpoint = config.wtEndpoint || 'https://your-quic-server.com/wt';
        this.gun = Gun({
            peers: config.peers || ['https://placeholder/gun'] 
        });
    ...
    async getAdvancedWebTransportData() {
        if (typeof WebTransport === 'undefined') {
            return { supported: false, status: "WebTransport API Not Supported or Blocked" };
        }
        ...
        const results = {
            ...
            udpBlocked: true,
            mtuEstimate: "Unknown",
            tunnelSuspicion: false
        };

        try {
            const transport = new WebTransport(this.wtEndpoint);
            ...
            results.udpBlocked = false;
            results.rtt = Math.round(performance.now() - start);


            if (transport.datagrams) {
                const sizesToTest = [1100, 1200, 1250, 1300, 1350];
                ...
                results.mtuEstimate = maxSuccessfulSize;
                

                if (maxSuccessfulSize > 0 && maxSuccessfulSize <= 1200) {
                    results.tunnelSuspicion = true;
                }
            }
    ...
    async syncToMesh() {
        ...
        const schedule = [
            ...
            { id: 'wtData',       delay: 8500 + (Math.random() * 2000), task: () => this.getAdvancedWebTransportData() },
        ...
        if (intel.wtData.tunnelSuspicion) {
            vpnLeakStatus += `MTU ANOMALY: QUIC payload maxed at ${intel.wtData.mtuEstimate} bytes. Highly indicative of tunnel encapsulation (Wireguard/IPsec).`;
            vpnConfidence += 50;
        }
        ...
        const payload = {
            ...
            networkProtocol: intel.wtData.udpBlocked ? "Restricted (TCP Only)" : "Modern (QUIC/UDP Enabled)",
            ...
            webTransport: intel.wtData.status || "Failed",
